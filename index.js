...
client.on("interactionCreated", async (interaction) => {
  ...
  if (commandName === "askvision") {
    const response = await runGeminiVisionPro(prompt, currentKeyIndex);
    await interaction.reply(response);
  }
  if (commandName === "help") {
    await interaction.reply("Ask me a question and I will answer it.");
  }
});
